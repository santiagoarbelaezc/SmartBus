<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,50:16213e,100:0f3460&height=130&section=header&animation=fadeIn&fontColor=e94560&fontSize=30&text=SmartBus" />
</div>

<div align="center">

[![Kotlin](https://img.shields.io/badge/Kotlin-1.9.0-7F52FF?style=flat-square&logo=kotlin&logoColor=white)](https://kotlinlang.org/)
[![Jetpack Compose](https://img.shields.io/badge/Jetpack_Compose-Latest-4285F4?style=flat-square&logo=jetpackcompose&logoColor=white)](https://developer.android.com/jetpack/compose)
[![Android](https://img.shields.io/badge/Min_SDK-24-3DDC84?style=flat-square&logo=android&logoColor=white)](https://developer.android.com/)
[![License](https://img.shields.io/badge/License-MIT-22c55e?style=flat-square)](LICENSE)
[![Status](https://img.shields.io/badge/Status-En%20Desarrollo-f59e0b?style=flat-square)]()

</div>

---

<h1 align="center">🚌 SmartBus — Android App</h1>
<h3 align="center">Aplicación nativa Android para compra de tiquetes y rastreo de buses en tiempo real</h3>

---

## 📌 Descripción

Módulo Android de **SmartBus**, desarrollado en **Kotlin + Jetpack Compose**. Permite buscar rutas, comprar tiquetes, rastrear el bus en tiempo real y acumular puntos de fidelización, todo desde el celular.

Arquitectura **Clean Architecture + MVVM**, conectado a una API REST centralizada desplegada en AWS.

---

## 📱 Pantallas

| Pantalla | Descripción |
|----------|-------------|
| Login / Register | Autenticación de usuario |
| Home | Panel principal con accesos rápidos |
| Búsqueda | Filtrado por origen, destino y fecha |
| Selección de Asiento | Mapa interactivo del bus |
| Pago | Integración con pasarela certificada |
| Mis Tiquetes | Activos e historial con código QR |
| Rastreo GPS | Bus en tiempo real sobre mapa |
| Puntos y Niveles | Bronce, Plata y Oro |
| Crédito de Viaje | Viaja ahora, paga después |
| Perfil | Datos del usuario y configuración |

---

## 🔧 Tech Stack

<div align="center">
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Jetpack_Compose-4285F4?style=for-the-badge&logo=jetpackcompose&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Hilt-2196F3?style=for-the-badge&logo=android&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Retrofit-48B983?style=for-the-badge&logo=square&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Google_Maps-4285F4?style=for-the-badge&logo=googlemaps&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" />
</div>

---

---


---

## 📄 Licencia

MIT License — ver [LICENSE](LICENSE) para detalles.

---

<div align="center">

👨‍💻 **Santiago Arbelaez Contreras**

*Junior Full Stack Developer · Universidad del Quindío*

<br>

<a href="https://www.linkedin.com/in/santiago-arbelaez-contreras-9830b5290/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<img width="10" />
<a href="https://portfolio-santiagoa.web.app/portfolio">
  <img src="https://img.shields.io/badge/✨_Portfolio-6C63FF?style=for-the-badge&logo=sparkles&logoColor=white" />
</a>
<img width="10" />
<a href="mailto:arbelaezz.c11@gmail.com">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,50:16213e,100:0f3460&height=100&section=footer&animation=fadeIn" />
</div>

